...
def runzap():
    ...
    for ifile in uploaded_files: 
        if ifile.filename.endswith(".html") :
            #print(ifile.filename)
            content = ifile.read().decode('utf-8')  # Read the file content as text
            upload_file = ifile.filename.replace("/","_")
            upload_file = store_folder+"/"+upload_file
            with open(upload_file,"w") as f:
                f.write(content)   
    session["dirname"] = store_folder
...
def passive_present():
    ...
    file_path = f'/home/jerry/zap/passive/{file_name}.txt'
    result_list = []
    with open(file_path,"r") as ff :
        result_list = json.load(ff)
...
def revised_response():
    ...
    return render_template("revised_response2.html",answer_list = answer_list,score = score,mostsimcontent = mostsimcontent,realfile = realfile)
